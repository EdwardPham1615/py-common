...
in its place. Both :mod:`py_common.runtime.grpc_metrics` and
:mod:`py_common.runtime.grpc_interceptors` do exactly that, so the mapping lives
here instead of being copied into each of them.

Private to ``py_common.runtime``; nothing here is re-exported.
"""
