...
:class:`~pycommon.security.keycloak.TokenClaims` into a handler signature —
...
from pycommon.security.keycloak import TokenClaims, unauthorized
from pycommon.security.requirements import AllOf, HasRole

if TYPE_CHECKING:
    from collections.abc import Awaitable, Callable

    from pycommon.config import BaseAppSettings
    from pycommon.security.keycloak import KeycloakTokenValidator
    from pycommon.security.requirements import Requirement
...
def internal_router(settings: BaseAppSettings, **kwargs: Any) -> APIRouter:
    ...
    :func:`~pycommon.testing.routes.assert_routes_protected` reports those
